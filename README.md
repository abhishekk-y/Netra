...
netra/
├── backend/
│   ├── api/                 # FastAPI REST and WebSocket controllers
│   ├── core/                # Configuration and dependency injection
│   ├── models/              # Pydantic and SQLAlchemy models
│   ├── services/            # Business logic (Ingestion, Topology, Detection)
│   ├── ml/                  # Model weights, inference scripts, feature engineering
│   ├── workers/             # Celery background tasks (PCAP processing)
│   └── tests/               # Pytest test suite
├── frontend/
│   ├── public/              # Static assets
│   ├── src/
│   │   ├── components/      # Reusable React components (Charts, Tables)
│   │   ├── pages/           # Dashboard, Topology Canvas, Settings
│   │   ├── store/           # Zustand state management
│   │   └── types/           # TypeScript interfaces
├── data/
│   ├── pcaps/               # Sample PCAP files for testing
│   └── models/              # Pre-trained ONNX/PyTorch models
├── deployment/
│   ├── docker-compose.yml   # Local deployment architecture
│   ├── kubernetes/          # K8s manifests
│   └── scripts/             # Initialization and migration scripts
...
| Model Architecture | Primary Purpose | Feature Space | Performance Target |
|--------------------|-----------------|---------------|--------------------|
| XGBoost | High-precision classification of known flow anomalies | 48-dim flow stats | > 99% TPR, < 1% FPR |
| Isolation Forest | Zero-day anomaly detection in entity behavior | Baselines (Packets, Bytes, Ports) | High recall, medium precision |
| Autoencoder | Complex payload structural anomaly detection | Byte-frequency distributions | O(1) latency per flow |
| GRU Networks | Attack phase transition forecasting | Sequence of categorical alerts | Predict next Tactic within 5s |
| Graph Neural Nets | Lateral movement target prediction | Topology edges and node risks | Sub-graph scoring < 50ms |
